...
class Trainer:
    ...
    def _eval_train(self, epoch):
        ...
        for i, data in ITER:
            context, resp = data['context'].to(self.device, non_blocking=True), data['resp'].to(self.device, non_blocking=True)
    ...
    def _pred_sample(self, n_sample):
        with torch.no_grad():
            ...
            prediction = self.model.predict([(samples['context'].to(self.device, non_blocking=True), samples['context_seg_id'].to(self.device, non_blocking=True))])
